function [varargout]=mat2wfdb64bit(varargin)
%


% UNDER CONSTRUCTION: REZA, 22 OCT 2020 


% [xbit]=mat2wfdb64bit(X,fname,Fs,bit_res,adu,info,gain,sg_name,baseline,isquant, isdigital)
%
...
if isdigital % digital input signal
    ...
    if (~isempty(find(baseline > 9223372036854775807))||~isempty(find(baseline < -9223372036854775808))) % baseline stored as int in wfdb library.
        error('Baseline field must lie between 2^-63 and 2^63-1 for this WFDB version'); % Prevent bit overflow
    end
...
if(isdigital)
    ...
    if rg==0 % Flatline signal. Manually set adc_gain or it will be infinite.
        ...
        if ((min_x>0) && (bit_res==32 || bit_res==64)) % All values are +ve, map with baseline = -2^31+1
        ...
        elseif((max_x<0) && (bit_res==32  || bit_res==64)) % All values are -ve, map with baseline = 2^31-1
        ...
        if(isquant)
            ...
            if (quantlevels>2^bit_res-2)
                if bit_res==64
                    disp(['The input signal has more quantization levels than 64 bits -1. ' ...
            ...
            if calcquant
                if ((min_x>0) && (bit_res==32 || bit_res==64)) % 32 bit +ve quant mapping
                    adc_gain=1/incmin;
                ...
                elseif((max_x<0) && (bit_res==32 || bit_res==64)) % 32 bit -ve quant mapping
                    adc_gain=1/incmin;
